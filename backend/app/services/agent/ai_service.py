...
from backend.app.services.agent.contact_service import get_contact_context, update_contact, get_contact_by_id
...
async def handle_aggiorna_contatto(contact_id, args):
    ...
    if data:
        ...
        return json.dumps({"status": "ok", "aggiornati": list(data.keys())})
...
async def handle_crea_appuntamento(contact_id, args):
    ...
    if result.data:
        ...
        try:
            ...
            gcal_location = "Google Meet (link in evento)" if modalita == "videocall" else "Via Dettori 3, Samatzai (SU)"
...
def _build_email_conferma(nome, data_label, modalita, location, meet_link):
    ...
    link_text = f'<p style="font-size:15px;color:#4b5563">Il link Google Meet verra inviato prima dell appuntamento.</p>' if modalita == "videocall" else f'<p style="font-size:15px;color:#4b5563">Stefano ti raggiungera all indirizzo indicato.</p>'

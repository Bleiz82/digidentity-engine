                    ...
                    <div className="bg-[#0A0A0A] border border-[#1F1F1F] rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        <div className="p-4 sm:p-6">
                            ...
                            <div className="space-y-3 text-sm">
                                ...
                                {selectedAppointment.modalita && <div className="flex items-center gap-2 text-[#9CA3AF]"><MapPin className="w-4 h-4" /><span>{selectedAppointment.modalita}{selectedAppointment.location && ` - ${selectedAppointment.location}`}</span>{selectedAppointment.modalita === "presenza" && selectedAppointment.location && <a href={`https://maps.google.com/?q=${encodeURIComponent(selectedAppointment.location)}`} target="_blank" rel="noopener noreferrer" className="ml-2 px-2 py-0.5 text-xs bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-lg hover:bg-blue-500/20 transition-colors">📍 Maps</a>}</div>}

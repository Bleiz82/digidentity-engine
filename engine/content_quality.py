...
try:
    ...
    LANGDETECT_DISPONIBILE = True
except ImportError:
    LANGDETECT_DISPONIBILE = False
...
async def analizza_contenuto(url: str) -> dict:
    """
    Analizza la qualità del contenuto e i segnali E-E-A-T per la visibilità AI.
    """
    ...
    lingua = None
    try:
        if LANGDETECT_DISPONIBILE and testo:
            lingua = detect(testo)
    except Exception:
        lingua = None
    ...
    headers_tags = soup.find_all(re.compile(r'^h[1-6]$'))
    if headers_tags:
        expet_score += 25
    else:
    ...
    bonus_lang = 5 if lingua == "it" else 0
    bonus_len = 5 if n_parole > 800 else 0
    bonus_faq = 5 if re.search(r"(FAQ|Domande frequenti)", html, re.IGNORECASE) else 0

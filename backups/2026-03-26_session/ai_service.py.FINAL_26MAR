...
async def generate_ai_response(conversation_id, contact_id, user_message, channel_type="whatsapp"):
    try:
        ...
        logger.info(f"[DIAG] finish_reason={response.choices[0].finish_reason}")
        logger.info(f"[DIAG] tool_calls={msg.tool_calls}")
        logger.info(f"[DIAG] tool_calls type={type(msg.tool_calls)}")
        logger.info(f"[DIAG] content preview={str(msg.content)[:300] if msg.content else None}")
        logger.info(f"[DIAG] messages_count={len(messages)} tools_count={len(TOOLS)}")
        logger.info(f"[DIAG] total_chars_sent={sum(len(str(m)) for m in messages)}")
        # === FINE DIAGNOSTICA ===

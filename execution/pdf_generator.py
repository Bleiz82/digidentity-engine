...
def _generate_premium_dashboard(scores: dict) -> str:
    """Deprecata — usa premium_pdf_style.generate_premium_dashboard()."""
    from execution.premium_pdf_style import generate_premium_dashboard
    return generate_premium_dashboard(scores)
...
def markdown_to_pdf(markdown_text: str, output_path: str, report_type: str = "free",
    ...
    Per PREMIUM usa il dark theme identico all HTML.
    """
    ...
    # PREMIUM Dark Theme
    from execution.premium_pdf_style import (
        get_premium_css,
        generate_premium_cover,
        generate_premium_dashboard as gen_dashboard,
    )
    ...
    _global_score = int(_score_match.group(1)) if _score_match else 0

    _premium_scores = {}
    score_patterns = [
        ("sito", "Sito Web"),
        ("velocita_mobile", "Velocit.* Mobile"),
        ("velocita_desktop", "Velocit.* Desktop"),
        ("seo_score", "SEO"),
        ("accessibilita", "Accessibilit"),
        ("best_practices", "Best Practices"),
        ("google_business", "Google Business"),
        ("facebook", "Facebook"),
        ("instagram", "Instagram"),
        ("reputazione_ai", "Reputazione AI"),
    ]
    for key, label in score_patterns:
        m = _re.search(label + r"[:\\s]*(\\d+)/100", markdown_text)
        if m:
            _premium_scores[key] = int(m.group(1))

    _gm = _re.search(r"PUNTEGGIO COMPLESSIVO.*?(\\d+)", markdown_text)
    if _gm:
        _premium_scores["globale"] = int(_gm.group(1))
    if not _premium_scores.get("globale") and _global_score:
        _premium_scores["globale"] = _global_score

    location = ""
    loc_match = _re.search(r"\\*\\*(.+?)\\*\\*\\s*\\|", markdown_text)
    if loc_match:
        location = loc_match.group(1)

    html_body = md_lib.markdown(markdown_text, extensions=[
        "tables", "fenced_code", "nl2br", "sane_lists",
    ])

    html_body = _re.sub(r"<(script|style)[^>]*>.*?</\\1>", "", html_body, flags=_re.DOTALL)

    cover = generate_premium_cover(company_name, date_str, location, _premium_scores.get("globale", _global_score))
    dashboard = gen_dashboard(_premium_scores)
...
<body>
    ...
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
    HTML(string=full_html, base_url="/app/").write_pdf(output_path)

    file_size = Path(output_path).stat().st_size / 1024
    logger.info(f"PDF Premium generato: {output_path} ({file_size:.1f} KB)")
    return output_path
